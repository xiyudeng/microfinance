...
%     pie = exp(theta1) ./ (exp(theta1)+exp(theta0)); 
    ...
    % at the first run, all approved and see the return
    bankA = ((p - (1/(1+c))) > 0);
    bankR = zeros([N,1]);
...
R_avg = bank_alg(N,t,c, ninfo, nempty)
...
%         idx_s = find(s == s_database(:,1:ninfo));
        ...
        s_database(idx_s,ninfo+2) = s_database(idx_s,ninfo+2)+return_count
        s_database(idx_s,ninfo+3) = s_database(idx_s,ninfo+3)+approve_count;
        
        % update prob
        s_database(idx_s,ninfo+1) = s_database(idx_s,ninfo+2)./s_database(idx_s,ninfo+3);        
    end
end
